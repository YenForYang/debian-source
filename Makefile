...
DOXYGEN = /c/tools/scoop/shims/doxygen
DOCZIP = icu-docs.zip
...
INSTALLED_BUILT_FILES = $(top_builddir)/config/Makefile.inc $(top_builddir)/config/pkgdata.inc $(top_builddir)/config/icu-config $(top_srcdir)/config/mh-msys-msvc $(EXTRA_DATA:%=$(DESTDIR)$(pkglibdir)/%) $(ALL_PKGCONFIG_FILES)
...
install-icu: $(INSTALLED_BUILT_FILES)
	...
	$(INSTALL_DATA) $(top_srcdir)/config/mh-msys-msvc $(DESTDIR)$(pkgdatadir)/config/mh-msys-msvc
...
config/icucross.inc: $(top_builddir)/icudefs.mk  $(top_builddir)/Makefile $(top_srcdir)/config/mh-msys-msvc
	@echo rebuilding $@
	@(grep '^CURR_FULL_DIR' $(top_builddir)/icudefs.mk ; \
	  grep '^CURR_FULL_DIR' $(top_srcdir)/config/mh-msys-msvc || echo ""; \
	   ) > $@
...
$(top_builddir)/config/icu-config: $(top_builddir)/Makefile $(top_srcdir)/config/icu-config-top $(top_srcdir)/config/icu-config-bottom $(top_builddir)/config/Makefile.inc $(top_srcdir)/config/mh-msys-msvc $(top_srcdir)/config/make2sh.sed
	-$(RMV) $@
	$(INSTALL_SCRIPT) $(top_srcdir)/config/icu-config-top $@
	chmod u+w $@
	@echo "# Following from icu/icu4c/source/config/Makefile.inc" >> $@
	LC_ALL=C $(SED) -f $(top_srcdir)/config/make2sh.sed < $(top_builddir)/config/Makefile.inc | grep -v '#M#' | uniq >> $@
	@echo "# Following from $(top_srcdir)/config/mh-msys-msvc" >> $@
	LC_ALL=C $(SED) -f $(top_srcdir)/config/make2sh.sed < $(top_srcdir)/config/mh-msys-msvc | grep -v '#M#' | uniq >> $@
